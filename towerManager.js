class TowerManager {
    ...
    run(roomInfo) {
        ...
        // Find invaders
        const invaders = roomInfo.room.find(FIND_CREEPS, { filter: (c) => !c.my });
        if (!invaders || !invaders.length) {
            return;
        }

        // Let's get our towers to attack the first invader
        const towers = roomInfo.room.find(FIND_MY_STRUCTURES, { filter: (s) => s.structureType === STRUCTURE_TOWER });
